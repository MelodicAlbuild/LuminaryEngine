...
public class GridInventoryMenu : MenuSystem
    {
        ...
        public GridInventoryMenu(InventoryComponent inventory, ResourceCache resourceCache, int x, int y, int width, int height, int zIndex = Int32.MaxValue)
        {
            _x = x;
            _y = y;
            _width = width;
        ...
        public override void Activate()
        {
            ...
            foreach (var item in _inventory.GetInventory())
            {
                ...
                var itemIcon = new ImageComponent(
                    _resourceCache.GetTexture(ItemManager.Instance.GetItem(item.Key).TextureId), // Replace with actual texture path
                ...
                var itemLabel = new TextComponent(
                    ...
                    currentX, currentY + CellHeight - 20, CellWidth, 20, zIndex: _zIndex - 1 // Render behind the icon
                );
